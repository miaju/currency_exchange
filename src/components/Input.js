...
export default function Input({
  ...
  return (
    <Form id="input">
      <Row>
        <Col>
          <FloatingLabel controlId="floatingInput" label="Currency I Have: ">
            <Form.Select
              value={state.from_name}
              onChange={(e) => setCountries("from_name", e.target.value)}
            >
        ...
        <Col>
          <FloatingLabel controlId="floatingInput" label="Currency I Want: ">
            <Form.Select
              value={state.to_name}
              onChange={(e) => setCountries("to_name", e.target.value)}
            >
              {countries?.map((country) => {
                const code = Object.keys(country.currencies)[0];
                return (
      ...
      <Row>
        <Col>
          <FloatingLabel controlId="floatingInput" label="Amount to Convert: ">
            <Form.Control
              type="number"
              pattern="^\d*(\.\d{0,2})?$"
              step="0.01"
      ...
      <Button id="con-bttn" onClick={handleSubmit}>
        Convert Currency
      </Button>
    </Form>
  );
