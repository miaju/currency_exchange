...
export default function Converter(props) {
  const { state, findData, setError, setCountries, setAmount } = useApplicationData();

  return (
    <div id="con">
      {state.errMes && <div id="err">{state.errMes}</div>}
      <Input state={state} setAmount={setAmount} setCountries={setCountries} setError={setError} findData={findData} countries={state.countries}/>
      <br/>
