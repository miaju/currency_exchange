...
export default function useApplicationData(){
  ...
  function getCountries() {
    return axios.get("https://restcountries.com/v3.1/all?fields=name,currencies")
      .then((res) => {
        const countries = (res.data.filter((country) => Object.keys(country.currencies).length > 0
        ).sort(function (a, b) {
  ...
  function getRate(from, to) {
    return axios.get(`https://v6.exchangerate-api.com/v6/${process.env.REACT_APP_EX_RATE_KEY}/latest/${from}`)
      .then((data) => {
  ...
  function getCurrency(country) {
    return axios.get(`https://restcountries.com/v3.1/name/${country}?fields=currencies`)
      .then((data) => {
